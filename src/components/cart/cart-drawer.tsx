...
export function CartDrawer({ open, onClose, config }: { open: boolean; onClose: () => void; config: PublicRestaurantConfig }) {
  ...
  const dialogRef = useModalDialog<HTMLElement>(open, closeDrawer);
  ...
  return (
    <div className="fixed inset-0 z-[60] flex justify-end bg-zinc-950/55 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Корзина">
      <button type="button" aria-label="Закрыть корзину" className="absolute inset-0 cursor-default" onClick={closeDrawer} />
