...
function SectionHeading({ icon, title, subtitle }: { icon: ReactNode; title: string; subtitle?: ReactNode }) {
  return <div className="mb-4 flex items-end justify-between gap-3"><div><div className="flex items-center gap-2 text-orange-600">{icon}<span className="text-xs font-bold uppercase tracking-[0.14em]">B-Bay рекомендует</span></div><h2 className="mt-1 text-xl font-extrabold tracking-tight text-zinc-950 dark:text-white sm:text-2xl">{title}</h2></div>{subtitle ? <div className="pb-1 text-sm text-zinc-500 dark:text-zinc-400">{subtitle}</div> : null}</div>;
}
...
export function MenuHome({ categories, items, isDemo }: { categories: Category[]; items: MenuItem[]; isDemo: boolean }) {
  ...
  <div className="mx-auto max-w-6xl px-4 pt-7 sm:px-6">
    ...
    <section data-section="menu" className={showHighlights ? "mt-9" : ""}><SectionHeading icon={<UtensilsCrossed size={16} />} title={activeCategory === "all" ? "Всё меню" : categories.find((category) => category.id === activeCategory)?.name ?? "Меню"} subtitle={<span className="flex items-center gap-2"><span>{filteredItems.length} поз.</span><span role="group" aria-label="Режим отображения блюд" className="inline-flex rounded-xl bg-white p-1 shadow-sm ring-1 ring-zinc-950/5 dark:bg-zinc-900 dark:ring-white/10"><button type="button" aria-label="Плитка" aria-pressed={viewMode === "grid"} onClick={() => setViewMode("grid")} className={`grid size-8 place-items-center rounded-lg transition ${viewMode === "grid" ? "bg-zinc-950 text-white dark:bg-white dark:text-zinc-950" : "text-zinc-400 hover:text-orange-600"}`}><LayoutGrid size={16} /></button><button type="button" aria-label="Лента" aria-pressed={viewMode === "list"} onClick={() => setViewMode("list")} className={`grid size-8 place-items-center rounded-lg transition ${viewMode === "list" ? "bg-zinc-950 text-white dark:bg-white dark:text-zinc-950" : "text-zinc-400 hover:text-orange-600"}`}><List size={17} /></button></span></span>} />
  ...
  <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-zinc-200 bg-white/95 pb-[env(safe-area-inset-bottom)] backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/95 sm:hidden"><div className="mx-auto flex max-w-md items-center justify-around px-4 py-2"><Link href="/" className="flex flex-col items-center gap-1 p-1 text-xs font-bold text-orange-600"><UtensilsCrossed size={20} />Меню</Link><button type="button" onClick={() => setCartIsOpen(true)} className="flex flex-col items-center gap-1 p-1 text-xs font-medium text-zinc-500 dark:text-zinc-400"><ShoppingBag size={20} />Корзина</button><a href={whatsAppLink ?? undefined} target="_blank" rel="noreferrer" className="flex flex-col items-center gap-1 p-1 text-xs font-medium text-zinc-500 dark:text-zinc-400"><MessageCircle size={20} />Связь</a></div></nav>
