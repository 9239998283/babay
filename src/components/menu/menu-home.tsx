...
function SectionHeading({ icon, title, subtitle }: { icon: ReactNode; title: string; subtitle?: string }) {
  return <div className="mb-4 flex items-end justify-between gap-3"><div><div className="flex items-center gap-2 text-orange-600">{icon}<span className="text-xs font-bold uppercase tracking-[0.14em]">B-Bay рекомендует</span></div><h2 className="mt-1 text-xl font-extrabold tracking-tight text-zinc-950 sm:text-2xl">{title}</h2></div>{subtitle ? <span className="pb-1 text-sm text-zinc-500">{subtitle}</span> : null}</div>;
}

function CartButton() {
  const { count, total } = useCart();
  if (count === 0) return null;
  return <Link href="/cart" className="fixed bottom-20 left-4 right-4 z-40 mx-auto flex max-w-2xl items-center justify-between rounded-2xl bg-zinc-950 px-5 py-3.5 text-white shadow-2xl shadow-zinc-950/25 transition hover:-translate-y-0.5 sm:bottom-6"><span className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-orange-500"><ShoppingBag size={18} /></span><span className="font-bold">Корзина <span className="text-zinc-400">· {count} шт.</span></span></span><span className="font-extrabold">{new Intl.NumberFormat("ru-RU").format(total)} ₽</span></Link>;
}
...
export function MenuHome({ categories, items, isDemo }: { categories: Category[]; items: MenuItem[]; isDemo: boolean }) {
  ...
  return <main className="min-h-dvh bg-[#f7f7f6] pb-36 text-zinc-900"><div className="mx-auto max-w-6xl px-4 pb-7 pt-4 sm:px-6 sm:pt-6">
    ...
    <div className="relative mt-5"><Search className="pointer-events-none absolute left-4 top-1/2 size-5 -translate-y-1/2 text-zinc-400" /><input value={query} onChange={(event) => { setQuery(event.target.value); setActiveCategory("all"); }} placeholder="Поиск по меню" className="h-13 w-full rounded-2xl border border-zinc-200 bg-white pl-12 pr-4 text-base outline-none shadow-sm transition placeholder:text-zinc-400 focus:border-orange-400 focus:ring-4 focus:ring-orange-100" /></div>
  </div>

  <nav aria-label="Категории меню" className="sticky top-0 z-30 border-y border-zinc-200/80 bg-[#f7f7f6]/95 py-3 backdrop-blur"><div className="mx-auto flex max-w-6xl gap-2 overflow-x-auto px-4 [scrollbar-width:none] sm:px-6"><button onClick={() => { setActiveCategory("all"); setQuery(""); }} className={`shrink-0 rounded-full px-4 py-2 text-sm font-bold transition ${activeCategory === "all" && !query ? "bg-zinc-950 text-white" : "bg-white text-zinc-600 ring-1 ring-zinc-200 hover:bg-zinc-100"}`}>Все</button>{categories.filter((category) => category.is_active).map((category) => <button key={category.id} onClick={() => { setActiveCategory(category.id); setQuery(""); }} className={`shrink-0 rounded-full px-4 py-2 text-sm font-bold transition ${activeCategory === category.id ? "bg-zinc-950 text-white" : "bg-white text-zinc-600 ring-1 ring-zinc-200 hover:bg-zinc-100"}`}>{category.name}</button>)}</div></nav>
  ...
  <div className="mx-auto max-w-6xl px-4 pt-7 sm:px-6">
    ...
    <section data-section="menu" className={showHighlights ? "mt-9" : ""}><SectionHeading icon={<UtensilsCrossed size={16} />} title={deferredQuery ? "Результаты поиска" : activeCategory === "all" ? "Всё меню" : categories.find((category) => category.id === activeCategory)?.name ?? "Меню"} subtitle={`${filteredItems.length} поз.`} />
      {filteredItems.length ? <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">{filteredItems.map((item) => <DishCard key={item.id} item={item} onOpen={setSelectedItem} />)}</div> : <div className="rounded-3xl border border-dashed border-zinc-300 bg-white px-6 py-12 text-center"><p className="font-bold text-zinc-950">Ничего не нашли</p><p className="mt-2 text-sm text-zinc-500">Попробуйте изменить запрос или выбрать другую категорию.</p></div>}
    </section>
  </div>
  <CartButton />
  <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-zinc-200 bg-white/95 pb-[env(safe-area-inset-bottom)] backdrop-blur sm:hidden"><div className="mx-auto flex max-w-md items-center justify-around px-4 py-2"><Link href="/" className="flex flex-col items-center gap-1 p-1 text-xs font-bold text-orange-600"><UtensilsCrossed size={20} />Меню</Link><Link href="/cart" className="flex flex-col items-center gap-1 p-1 text-xs font-medium text-zinc-500"><ShoppingBag size={20} />Корзина</Link><a href={whatsAppLink ?? undefined} target="_blank" rel="noreferrer" className="flex flex-col items-center gap-1 p-1 text-xs font-medium text-zinc-500"><MessageCircle size={20} />Связь</a></div></nav>
